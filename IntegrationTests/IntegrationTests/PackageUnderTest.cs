...
/// <summary>
/// Stages the freshly built Sbom package into a private feed, once per test run.
/// </summary>
public static class PackageUnderTest
{
    ...
    static BuiltPackage Stage()
    {
        ...
        var packages = Directory.GetFiles(nugets, "Sbom.*.nupkg").ToList();

        if (packages.Count != 1)
        {
            throw new(
                $"Expected exactly one Sbom package in '{nugets}', found {packages.Count}. " +
        ...
        // staged.
        var feed = Path.Combine(Path.GetTempPath(), "sbom-it-feed", Guid.NewGuid().ToString("N"));
    ...
    /// <summary>
    /// "Sbom.0.1.0.nupkg" gives "0.1.0". Read from the filename rather than from
